...
function* createUser(){
    yield takeEvery('CREATE_NEW_USER', CreateNewUserFinish);
}
...
const updateTaskInfo = (userid, task_id, title, overview) => {
    let bodydata = JSON.stringify({
...
const deleteTaskInfo = (userid, task_id) => {
...
export function* CreateNewTaskFinish(){
    try{
        ...
        const tasks = yield createnewTask(state.getTableData.user_id);
...
export function* UpdateTaskFinish(action){
    try{
        const state = yield select();
        const tasks = yield updateTaskInfo(state.getTableData.user_id, action.task_id, action.title, action.overview);
        yield put({type: 'DATA_LOADED', data: tasks, loading: false})
    }catch(error){
        yield put({type: 'DATA_LOAD_FAILURE', error})
    }
...
export function* DeleteTaskFinish(action){
    try{
        const state = yield select();
        const tasks = yield deleteTaskInfo(state.getTableData.user_id, action.task_id);
        yield put({type: 'DATA_LOADED', data: tasks, loading: false})
    }catch(error){
        yield put({type: 'DATA_LOAD_FAILURE', error})
    }
...
export function* getTableTasksfromDB(){
    try{
        const state = yield select();
        const tasks = yield fetchTasks(state.getTableData.user_id);
        yield put({type: 'DATA_LOADED', data: tasks, loading: false})
    }catch(error){
        yield put({type: 'DATA_LOAD_FAILURE', error})
    }
